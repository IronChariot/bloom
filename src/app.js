...
function update(next) {
  ...
  document.querySelector('#save-status').textContent = state.graph ? state.dirty ? 'Unsaved changes · recovery on' : state.path ? 'Saved to file' : 'Recovery on · not saved to file' : 'Open a board to begin';
  ...
  if (!state.graph) document.querySelector('#empty').innerHTML = `<img src="assets/icon.svg" alt=""><h1>A little room for ideas.</h1><p>Start a new board or pick up where you left off.</p><button class="primary-button" data-command="new">New brainstorm</button><button class="subtle-button" data-command="open">Open a board…</button>${state.recent.slice(0, 3).map(p => `<button class="recent-open" data-recent="${esc(p)}">${esc(p.split(/[\\/]/).pop())}</button>`).join('')}`;
...
function fileMenu() {
  ...
  panels.innerHTML = `<div class="popover file-menu">${item('new', 'New brainstorm', 'Ctrl N')}${item('open', 'Open…', 'Ctrl O')}<div class="menu-rule"></div>${item('save', 'Save', 'Ctrl S')}${item('saveAs', 'Save as…', 'Ctrl Shift S')}${item('export', 'Export JSON Canvas…')}<div class="menu-rule"></div><div class="menu-label">Recent boards</div>${state.recent.length ? state.recent.map(p => `<button class="menu-item" data-recent="${esc(p)}"><span class="recent-name">${esc(p.split(/[\\/]/).pop())}</span></button>`).join('') : '<div class="menu-item" style="color:#aaa">Saved boards will appear here</div>'}<div class="menu-rule"></div>${item('close', 'Close board', 'Ctrl W')}</div>`;
}
...
function renderSession() {
  ...
  panels.innerHTML = `<section class="popover session-panel" aria-label="Agent session"><div class="panel-heading">Think together<button data-close-panel aria-label="Close panel">${icon('close')}</button></div><div class="session-status"><span class="session-indicator${sessionInfo.enabled ? ' active' : ''}"></span>${sessionInfo.enabled ? 'Local session available' : 'Agent access paused'}</div><p>Connect your agent to this open board. It can read ideas, see the canvas, and make changes alongside you.</p><div class="endpoint">${esc(sessionInfo.url || 'Starting session…')}</div><button class="primary-button" id="copy-session">${icon('copy')}Copy MCP connection</button><button class="subtle-button" id="pause-session">${sessionInfo.enabled ? 'Pause agent access' : 'Resume agent access'}</button><div class="activity-title">Activity</div>${last ? `<div class="activity-item">Agent<span>${esc(last.message)}</span></div>` : '<p style="font-size:11px;margin-bottom:7px">No agent activity yet. Paste the connection into your agent’s MCP settings.</p>'}${state.activity.slice(-4).reverse().map(a => `<div class="activity-item">${esc(a.actor)}<span>${esc(a.message)}</span></div>`).join('')}</section>`;
}
...
document.addEventListener('click', async e => {
  ...
  const actions = { 'file-toggle': fileMenu, 'session-toggle': showSession, 'help-toggle': showHelp, 'zoom-in': () => zoom(1.15), 'zoom-out': () => zoom(1 / 1.15), 'zoom-value': () => zoom(1 / view.zoom), fit, tidy, 'copy-session': async () => { await act('copySession'); toast('MCP connection copied.'); }, 'pause-session': async () => { sessionInfo = await api.command('sessionToggle', !sessionInfo.enabled); renderSession(); } };
  actions[button.id]?.();
});
