...
Changes are saved to D1. Active visible tabs check for edits every two seconds. After thirty seconds without input and no active selections they check every fifteen seconds; hidden tabs stop checking. Returning to a tab, interacting after idle, or reconnecting checks immediately. Failed requests back off up to sixty seconds. Member last-seen is included in the same check and written at most once per twenty seconds per person. Selection and editing presence is independent, with debounced selection changes and a fifteen-second heartbeat while selected or editing. An unchanged response contains small status/presence fields, without the graph, history or image. Graph mutations use an atomic revision check, so two competing writes cannot silently overwrite one another. An edit conflict preserves text drafts for review. Undo and redo operate on the shared board history, including agent edits, rather than each person's private history.
...
**Upload board**, next to Download, restores a `.bloom` download or supported text-node JSON Canvas file into the current board. Its title, graph, styles and petals replace the previous contents as one shared undo/redo step; the board ID, members, sharing links and agent grants stay in place. The file-menu import and Ctrl+O use the same flow. With no board open, importing creates a board. Pending text edits are saved before the picker opens. Invalid/oversized files and stale revisions are rejected without replacing the board. `node tests/web-import.mjs` verifies an actual browser download/upload roundtrip, undo/redo and failure cases against the real API handlers with isolated SQLite storage.
...
Other tabs’ selected blobs and links receive solid red outlines and nearby display-name labels. Text editing first acquires an exclusive per-node server lease, then opens the editor using fresh text. A lease lasts 45 seconds, renews every 15 seconds, and is released after save/cancel, board switch or tab close. If connectivity is lost, the local editor stops after a conservative 40-second deadline; unsaved drafts remain available to retry, copy or discard. Selection is advisory; moving/recolouring a locked node and editing unrelated nodes remain allowed. Text replacement/deletion (including agent edits, Undo and import) checks the lease in the same atomic database transaction as the graph write. Presence and locks never advance graph/content/layout revisions or appear in Undo history. Migration `0005_slimy_titania.sql` creates these ephemeral tables.

Collaboration checks: `node --test tests/presence.test.js` and `node tests/web-presence.mjs` from the repository root cover concurrent editors (including separate tabs for the same user), lock expiry, write races, agent protection, draft recovery, labels, and disconnect cleanup. The live MCP smoke test uses a temporary lock on a disposable board.
...
Tools: `claim_board`, `list_boards`, `get_board`, `edit_board`, `get_board_image`. Specify the granted boardId and use the content/layout preconditions from get_board for atomic edits, or expectedRevision for a strict whole-board check. Legacy board-specific bearer connections retain their four original tools and `bloom://board/current` / `bloom://board/canvas` resources. Browser WebMCP also exposes get_board and edit_board through the signed-in API when supported.

Remote MCP reads default to compact IDs/text/connections, with optional layout, activity and participants. Select `nodeIds` for targeted verification with incident edges and explicit partial scope. Edits default to exact committed revision receipts containing added/updated entities and deleted IDs. Full reads/responses remain opt-in. Reads also support sinceRevision for net changes with an explicit resync response when retained history is unavailable. See the [batch example and response formats](AGENT_CONNECTION.md#efficient-brainstorming-mcp-05). The existing browser HTTP API and browser WebMCP continue to use full state.

Connections combine solid/dotted patterns with no, forward, reverse or two-way arrowheads. Sprouting defaults to a solid forward arrow. Ctrl-click (also Cmd or Shift) selects multiple blobs or multiple connections; changing a style affects the selection as one undoable batch. Right-click a blob for coloured, emoji or comment petals. Comments support Shift+Enter line breaks and Enter to confirm, with server-stamped author/date on hover. Each blob holds eight petals; drag them between the 45-degree positions to shift neighbours with animation. Click an existing petal to edit, recolour or delete it. `node tests/web-petals.mjs` from the repository root exercises these interactions in Chrome against an isolated test server.
