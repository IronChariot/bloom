...
Remote MCP reads default to compact IDs/text/connections, with optional layout, activity and participants. Select `nodeIds` for targeted verification with incident edges and explicit partial scope. Edits default to exact committed revision receipts containing added/updated entities and deleted IDs. Full reads/responses remain opt-in. See the [batch example and response formats](AGENT_CONNECTION.md#efficient-brainstorming-mcp-05). The existing browser HTTP API and browser WebMCP continue to use full state.
