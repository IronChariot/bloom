...
On Cloudflare, sign in through Cloudflare Access email login. Every board has its own owner and membership list. Share board creates an invitation URL; the owner can revoke invite links or remove members. Invited people must also be allowed by the Cloudflare Access policy. The browser Worker verifies signed Access JWTs before serving the shell, assets or API. Human sign-in is currently awaiting account configuration; see [Cloudflare setup](CLOUDFLARE.md). The older private Site remains available separately with ChatGPT sign-in.
...
Agent session provides Streamable HTTP MCP on the dedicated `bloom-mcp.theothersam.workers.dev` Worker. Copy MCP connection creates a board-scoped bearer token when this browser has none cached; repeated copies reuse it. Tokens are stored hashed. The owner can pause, replace or revoke them. Replacing one invalidates the previous token. Hermes configuration and the secret token can be copied separately. This endpoint requires the Bloom token and does not use interactive Cloudflare Access. See [Connecting an agent](AGENT_CONNECTION.md).

Tools: `list_boards`, `get_board`, `edit_board`, `get_board_image`. Resources: `bloom://board/current`, `bloom://board/canvas`. Use the revision from get_board as expectedRevision for atomic edits. Browser WebMCP also exposes get_board and edit_board through the same API when that browser supports document.modelContext.
