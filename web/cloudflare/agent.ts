import { access, agentConnection } from '../lib/boards.js';
...
export default {
  async fetch(request: Request, env: any) {
    ...
    if (!/^Bearer (?:[A-Za-z0-9]{64}|bloom_agent_[a-f0-9]{64})$/.test(authorization)) return new Response('A Bloom connection key or board token is required.', { status: 401, headers: { 'WWW-Authenticate': 'Bearer realm="Bloom"' } });
    return withIdentity(null, async () => {
      let connection;
      try {
        if (authorization.startsWith('Bearer bloom_agent_')) {
          connection = await agentConnection(request);
          if (!connection) return new Response('Invalid Bloom connection key.', { status: 401 });
        } else {
      ...
      return handleMcp(new Request(request, { body }), connection?.owner);
    });
  }
};
